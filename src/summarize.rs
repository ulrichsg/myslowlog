...
pub fn summarize(entries: &Vec<LogEntry>) -> Summary {
    let max_execution_time = entries
        .iter()
        .fold(0, |acc, val| max(acc, val.query_time.num_microseconds().unwrap()));

    let total_execution_time = entries
        .iter()
        .fold(0, |acc, val| acc + val.query_time.num_microseconds().unwrap());
