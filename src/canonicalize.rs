...
use sqlparser::ast::{
    Cte, DateTimeField, Expr, Join, JoinConstraint, JoinOperator, OrderByExpr, Select, SelectItem,
...
fn canonicalize_query(query: Query) -> Query {
    let ctes = query.ctes.into_iter().map(canonicalize_cte).collect();
    let order_by = query
        .order_by
        .into_iter()
...
fn canonicalize_select(select: Select) -> Select {
    ...
    let from = select
        ...
        .collect();
    let group_by = select.group_by.into_iter().map(canonicalize_expr).collect();
...
fn canonicalize_table_with_joins(twj: TableWithJoins) -> TableWithJoins {
    let joins = twj.joins.into_iter().map(canonicalize_join).collect();
    TableWithJoins {
...
fn canonicalize_join_operator(operator: JoinOperator) -> JoinOperator {
    match operator {
        JoinOperator::Inner(constraint) => {
            JoinOperator::Inner(canonicalize_join_constraint(constraint))
        }
        JoinOperator::LeftOuter(constraint) => {
            JoinOperator::LeftOuter(canonicalize_join_constraint(constraint))
        }
        JoinOperator::RightOuter(constraint) => {
            JoinOperator::RightOuter(canonicalize_join_constraint(constraint))
        }
        JoinOperator::FullOuter(constraint) => {
            JoinOperator::FullOuter(canonicalize_join_constraint(constraint))
        }
        default => default,
    }
}
...
fn canonicalize_values(values: Values) -> Values {
    let result = values
        .0
        .into_iter()
        .map(|vec| vec.into_iter().map(canonicalize_expr).collect())
        .collect();
    Values(result)
}
...
fn canonicalize_expr(expr: Expr) -> Expr {
    let map_exprs = |exprs: Vec<Expr>| exprs.into_iter().map(canonicalize_expr).collect();
    let map_boxed_expr = |boxed: Box<Expr>| Box::new(canonicalize_expr(*boxed));
    let map_boxed_query = |boxed: Box<Query>| Box::new(canonicalize_query(*boxed));
    match expr {
